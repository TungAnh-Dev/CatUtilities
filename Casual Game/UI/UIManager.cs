...
public class UIManager : Singleton<UIManager>
{
    ...
    public T OpenUI_Immediately<T>() where T : UICanvas
    {
        UICanvas canvas = GetUI<T>();
        canvas.Setup();
        canvas.Open();
    ...
    public void CloseUI_Directly<T>() where T : UICanvas
    {
        if (IsOpened<T>())
        {
            GetUI<T>().CloseDirectly();
    ...
    public void CloseAllExcept(params System.Type[] exceptions)
    {
        foreach (var item in uiCanvas)
        {
            if (item.Value != null && item.Value.gameObject.activeInHierarchy)
            {
                // If the canvas type is not in the exceptions list, close it
                if (!System.Array.Exists(exceptions, exception => exception == item.Key))
                {
                    item.Value.CloseDirectly();
                }
            }
